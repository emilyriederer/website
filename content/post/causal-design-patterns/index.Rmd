...
Software engineers study design patterns^[A concept popularized by the 1994 book *Design Patterns: Elements of Reusable Object-Oriented Software*. See more on [Wikipedia](https://en.wikipedia.org/wiki/Design_Patterns).] to help them recognize archetypes in the specific problems they are facing, to discuss such challenges with a shared vocabulary, and to reuse tried-and-true paradigms to address these problems. 

Statistics also has plenty of examples of prototypical analyses; however, sometimes these frameworks become siloed within a specific field of study (e.g. epidemiology, economics, biostatistics) and clouded in domain-specific language that masks their general applicability^[Two other examples of this are categorical data analysis methods associated only with text mining and time-to-event analyses covered predominately in survival analysis]. This can make it hard for practitioners outside of these fields to easily survey, learn, and apply these methods.
