...
description: "A right-sized solution to automated data monitoring, alerting, and reporting using R (`pointblank`, `projmgr`), GitHub (Actions, Pages, issues), and Slack"
author: "Emily Riederer"
date: "2021-08-26"
categories: [rstats, data, elt]
image: "featured.png"
---

Data quality monitoring is an essential part of any data analysis or business intelligence workflow. As such, an increasing number of promising tools^[Just to name a few: dbt, datafold, Soda, Great Expectations, and Monte Carlo] have emerged as part of the [Modern Data Stack](https://moderndatastack.xyz/) to offer better orchestration, testing, and reporting. 
...
With this information, we can use `projmgr` to connect to a GitHub repository^[Assuming you have your personal access token set per the documentation: https://emilyriederer.github.io/projmgr/articles/github-pat.html].
...
- Exposes the GITHUB personal access token we need for `projmgr` to be able to write issues to our repository^[This assumes that within GitHub, the PAT is defined as a secret called GH_PAT. Curiously, GitHub does not allow secrets that start with the word "GITHUB". Who knew? Additionally, depending on the privacy level of your repository, you might not need a PAT to make issues and could skip this step.]
