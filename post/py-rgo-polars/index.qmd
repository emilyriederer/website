...
Equally important to the ergonomics is what design features both packages forego:^[In both of these cases, `pandas` is the elephant-in-the-room foil. I say this not to criticize `pandas` but just to highlight specific features of its design which may be particularly unappealing or unintutive to those coming from other places.]

- Modification in-place: `dplyr` and `polars` functions return a new object versus modifying an existing dataset. This isn't always true with `pandas` where it can be easier to accidentally modify and existing dataset "in place" which makes code less idempotent^[Meaning you can't get the same result twice because if you rerun the same code the input has already been modified] and more error prone
- Indexes: I don't want to belabor this or criticize `pandas`, but for R users, `pandas` indexes may seem like an elusive chimera that's sometimes column-like, sometimes-not, and creates another property to mentall track about a dataset at each step
